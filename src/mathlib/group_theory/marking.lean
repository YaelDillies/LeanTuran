...
@[simp] lemma norm_to_marked_group (s : S) :
  ‖((to_marked_group (m (free_group.of s))) : marked_group m)‖ = 1 := sorry
...
lemma gen_norm_le_one_sub {H : set G}  {m' : group_marking G H} {s : marked_group m'} (sh : s ∈ H) : ‖s‖ ≤ 1 :=
sorry

lemma gen_set_mul_right_sub {H : set G} {s : G} {m' : group_marking G H} (sh : s ∈ H) (g : marked_group m') :
  ‖g * s‖ ≤ ‖g‖ + 1 :=
sorry

lemma gen_set_mul_right'_sub {H : set G} {s : G} {m' : group_marking G H} (sh : s ∈ H) (g : marked_group m')
  {n : ℝ} (hg : ‖g‖ ≤ n) : ‖g * s‖ ≤ n + 1 :=
sorry

lemma gen_set_mul_left_sub {H : set G} {m' : group_marking G H} (g s: marked_group m') (sh : s ∈ H) :
  ‖s * g‖ ≤ ‖g‖ + 1 :=
sorry

lemma gen_set_mul_left'_sub {H : set G} {m' : group_marking G H} (g s : marked_group m')  (sh : s ∈ H) {n : ℝ}
  (hg : ‖g‖ ≤ n) : ‖s * g‖ ≤ n + 1 :=
sorry

lemma dist_one_iff_sub {H : set G} {m' : group_marking G H} (x y : marked_group m') :
  dist x y = 1 ↔ (∃ s ∈ H, x * s = y) ∨ ∃ s ∈ H, y * s = x :=
sorry

lemma gen_div_left_sub {H : set G} {m' : group_marking G H} (x : marked_group m') (hx : x ≠ 1) :
