...
begin
  ...
  obtain ⟨c,hc⟩:=nat.exists_eq_succ_of_ne_zero Ac', rw [hc, nat.succ_eq_add_one] at this,
...
lemma sdiff_inter_disj (A B C:finset α) (h: B⊆A): disjoint (B ∩ C)  ((A\B)∩ C):=
begin
...
-- if G has no s-clique then nor does the univ se
...
begin
  ...
  exact sdiff_inter_disj A B _ hB,
end
...
begin
  ...
  by_cases hnem: A.nonempty,{
    ...
    -- res nbhd of x is (s+2)-clique free (since A is (s.succ+2)-clique free
