...
@[reducible]
def ind (A : finset α) : simple_graph α := { adj := λ x y, G.adj x y ∧ x ∈ A ∧ y ∈ A }
...
begin
  ext,
  simp only [map_adj, comap_adj, function.embedding.coe_subtype, set_coe.exists, mem_coe,
    subtype.coe_mk, exists_prop],
  split,
  { rintro ⟨a,h1,b,h2,h3,h4,h5⟩,rw [←h4,←h5], exact ⟨h3,h1,h2⟩},
  {rintro ⟨h,h1,h2⟩, exact ⟨x,h1,x_1,h2,h,rfl,rfl⟩,},
end
...
lemma empty_of_diff_parts {P : finpartition A} {i j : ℕ}(hi: i∈range(M.t)) (hj: j∈range(M.t)) (hne:i≠j):
...
def edges_inside (P : finpartition A) : finset(sym2 α):=(range(M.t)).bUnion (λi, (G.ind (M.P i)).edge_finset)

--so counting edges inside M is same as summing of edges in induced parts (since parts are disjoint..)
lemma edge_mp_count {P : finpartition A} : (G.edges_inside M).card = ∑ i in range(M.t),(G.ind (M.P i)).edge_finset.card:=
begin
...
lemma ind_nbhd_mem {A : finset α} {v : α} : v∈ A → (G.ind A).neighbor_finset v =  A ∩ G.neighbor_finset v:=
begin
...
lemma ind_deg_mem {A : finset α} {v : α} : v∈ A → (G.ind A).degree v= G.deg_on A v:=
begin
...
-- so degrees of v in the induced graph are deg_on A v or 0 depending on whether or not v ∈ A
lemma ind_deg {A :finset α}{v:α} : (G.ind A).degree v = ite (v∈A) (G.deg_on A v) (0):=
begin
  unfold degree,
...
-- so degree sum over α in the induced subgraph is same as sum of deg_on over A
-- both count 2*e(G[A])
lemma ind_deg_sum {A :finset α}: ∑v, (G.ind A).degree v = ∑v in A,(G.deg_on A v):=
begin
...
lemma deg_bipart_mem {A : finset α} {v : α} (h: v∈ A) : (G.bipart A).degree v = (G.deg_on A vᶜ):=
begin
...
lemma nbhd_bipart_not_mem {A : finset α} {v : α} (h: v ∉ A) :  (G.bipart A).neighbor_finset v = A ∩ G.neighbor_finset v:=
begin
  ext, simp only [mem_res_nbhd, mem_neighbor_finset], tidy; tauto,
end

-- if v∉ A then in the bipartite graph deg v is the deg_on to A
lemma deg_bipart_not_mem {A : finset α} {v : α} (h: v ∉ A) :  (G.bipart A).degree v = G.deg_on A v:=
begin
...
begin
  ...
  exact G.deg_on_add (subset_univ A),
end
...
@[reducible]
def bUnion (P : finpartition A) : simple_graph α := {
adj:= λ v w, ∃i ∈ range(M.t), (G.ind (M.P i)).adj v w,
symm:= by obviously,
loopless:= by obviously,}

@[reducible]
def disJoin (P : finpartition A) : simple_graph α:={
adj:= λ v w , (G.adj v w) ∧ (∃ i ∈ range(M.t), v∈(M.P i) ∧w∈ (M.P i)),
symm:= by obviously,
loopless:= by obviously,}

-- the two versions of "union of induced disjoint parts" are the same
lemma bUnion_eq_disJoin_sum (P : finpartition A) : G.bUnion M = G.disJoin M:=
begin
...
lemma edges_inside_eq (P : finpartition A) : (G.bUnion M).edge_finset = (G.edges_inside M):=
begin
...
lemma disJoin_sub (P : finpartition A) : (G.disJoin M)≤ G:=λ _ _ h, h.1

-- G with the internal edges removed is G ⊓ (mp M)
lemma sdiff_with_int {P : finpartition A} (h: M.A =univ)  : G\(G.disJoin M) = G⊓(mp M):=
begin
  ext x y,dsimp,
...
lemma self_eq_disJoin_ext_mp {M :finpartition A} (h: M.A=univ) : G = (G.disJoin M) ⊔ (G⊓(mp M)):=
begin
...
lemma disJoin_edge_help {M :finpartition A} {v w : α} {i : ℕ}: i ∈ range(M.t) → v ∈ (M.P i) →  w ∈ M.A →
...
lemma disJoin_edge_help' {M :finpartition A} (h: M.A=univ) {v w:α}{i:ℕ}: i∈ range(M.t) → v ∈ (M.P i) →
...
lemma disJoin_deg_sum {M :finpartition A} (h: M.A=univ) :
...
lemma disJoin_edge_sum {M :finpartition A} (h: M.A=univ) :
...
lemma ind_edge_count {A : finset α}: ∑  v in A, G.deg_on A v = 2* ((G.ind A).edge_finset.card ) :=
begin
