...
def Turan_Graph  (t n : ℕ):  simple_graph (fin n) :=
begin
  induction t with s hs, 
  exact ⊥, 
  set f:ℕ → ℕ:= λa, a*(n+1-a)+ turan_numb s (n+1-a),
  set vals:= image f (range(n+1)),
...
--lemma turan_example : ∀t:ℕ, ∃ G:simple_graph α, G.edge_finset_card = turan_numb t (card α)  ∧ G.clique_free (t+1):=
--begin
