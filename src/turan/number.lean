...
lemma tn_turan_num'  (t n : ℕ) : turan_num' t n + 2* turan_num t n = n^2:=
begin
  obtain rfl | ht := @eq_zero_or_pos _ _ t,
  { simp only [turan_num, turan_num'],
    simp only [zero_add, mod_zero, nat.div_zero, zero_pow two_pos, sub_zero, mul_zero, one_pow,
      mul_one, zero_mul, one_mul, two, mul_tsub, sq],
  ...
  { rw hn at *,
    simp only [*, nat.zero_div, zero_pow', ne.def, bit0_eq_zero, nat.one_ne_zero, not_false_iff,
      mul_zero, zero_add, zero_mul, choose_zero_succ] at * },
  obtain hb' | hb' := nat.eq_zero_or_pos b,
  { rw hb' at *,
    rw add_zero at n1,
    simp only [tsub_zero, add_tsub_cancel_right, mul_zero, add_zero, zero_mul, choose_zero_succ,
      mul_left_comm 2, two, ←n1],
    ring_nf,
...
lemma tn_turan_num'_2 (t n : ℕ) : 2*(turan_num t n) = n^2 - (turan_num'  t n):=
begin
  rw ← tn_turan_num' t n,
...
variables {α : Type*}[fintype α][nonempty α][decidable_eq α] {A : finset α}
